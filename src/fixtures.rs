...
use crate::dynamodb::qruuid::gen_qr_id;
...
pub fn get_fixture() -> Result<QrGroup, Box<dyn Error>> {
    ...
    let qr_item_1 = QrItem{ name: "Garden hose".to_string() };
    let qr_item_2 = QrItem{ name: "Soldering iron".to_string() };
    let qr_item_3 = QrItem{ name: "Laptop".to_string() };
    let qr_item_4 = QrItem{ name: "Games".to_string() };
    let qr_item_5 = QrItem{ name: "Toys".to_string() };
